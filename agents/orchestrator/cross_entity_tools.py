...
def make_cross_entity_tools(user_id: str):
    ...
    async def link_entities(
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Linked {from_type}/{from_id} --[{rel_type}]--> {to_type}/{to_id}",
                "link": result,
            }
        except Exception as e:
            return {"success": False, "message": f"Error linking entities: {e}"}
    ...
    async def get_entity_graph(entity_type: str, entity_id: str) -> dict:
        ...
        try:
            result = await md_storage.get_entity_connections(user_id, entity_type=entity_type, entity_id=entity_id)
            if result is None:
                return {"success": False, "message": f"Entity not found: {entity_type}/{entity_id}"}
            return {"success": True, **result}
        except Exception as e:
            return {"success": False, "message": f"Error getting entity graph: {e}"}
