...
def identify_face_from_url_tool(user_id: str, image_url: str) -> dict:
...
def store_person_face_tool(user_id: str, person_id: str, image_url: str) -> dict:
...
def get_person_tool(user_id: str, person_id: str) -> dict:
...
def list_persons_tool(user_id: str, limit: int | None = 50) -> dict:
    """List all saved persons for the current user."""
    try:
        persons = graph_db.list_person_nodes(user_id, limit or 50)
...
def delete_person_tool(user_id: str, person_id: str) -> dict:
...
def search_person_tool(user_id: str, search_term: str) -> dict:
    ...
    try:
        ...
        persons = graph_db.search_persons(user_id, search_term)
        return {
            "success": True,
            "count": len(persons),
            "search_term": search_term,
...
def add_relationship_tool(
    ...
    try:
        from_results = graph_db.search_persons(user_id, from_person_name)
        if not from_results:
            return {"success": False, "message": f"Person '{from_person_name}' not found. Create them first."}

        to_results = graph_db.search_persons(user_id, to_person_name)
        if not to_results:
...
def get_relationships_tool(user_id: str, person_name: str) -> dict:
    """Get all relationships for a person in the knowledge graph."""
    try:
        results = graph_db.search_persons(user_id, person_name)
        if not results:
